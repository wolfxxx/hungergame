...
// === /STROKE_WARP_GUARD ===
  ...
  return char === 'W' || char === 'X';
    ...
    function findTiles(chr){
      const maze = getCurrentMaze();
      const out = [];
      for(let y=0; y<maze.length; y++){
        for(let x=0; x<maze[0].length; x++){
          if(maze[y][x]===chr) out.push({x,y});
        }
      }
      return out;
    }
    function nearestGhostHome(tx, ty){
      const homes = findTiles('G');
      if(!homes.length) return pen || {x:tx, y:ty};
      let best = homes[0], bestD = Infinity;
      for(const h of homes){
        const d = manhattan({x:tx, y:ty}, h);
    ...
    function bfsNextStep(sx, sy, tx, ty, passGate=false){
      ...
      const W = maze[0].length, H = maze.length;
      const wrapX = (x)=> (x < 0 ? W-1 : (x >= W ? 0 : x));
      const startX = wrapX(sx), startY = Math.max(0, Math.min(H-1, sy));
      const targetX = wrapX(tx), targetY = Math.max(0, Math.min(H-1, ty));
      ...
      while(q.length){
        ...
        for(const [nx,ny,name] of neigh(x,y)){
          const key=nx+","+ny;
          if(seen.has(key)) continue;
          seen.add(key);
          parent.set(key,[x,y,name]);
          q.push([nx,ny]);
        }
      }
      let key=targetX+","+targetY;
      if(!(startX===targetX && startY===targetY) && !parent.has(key)) return null;
      let prev=null;
      while(parent.has(key)){
        prev = parent.get(key);
        if(prev[0]===startX && prev[1]===startY) return prev[2];
        key = prev[0]+","+prev[1];
      }
      return null;
    }
    // Fallback greedy chooser
    function chooseDirection(fromDir, tx, ty, target, frightened=false, passGate=false, allowReverse=false){
      const maze = getCurrentMaze();
      const W = maze[0].length, H = maze.length;
      const wrapX = (x)=> (x < 0 ? W-1 : (x >= W ? 0 : x));
      const options = [];
      for(const [name, d] of Object.entries(DIRS)){
        if(!allowReverse && opposite(fromDir)===name) continue;
      ...
      if(options.length===0){
        if(fromDir){
          const rev = opposite(fromDir);
          const d=DIRS[rev];
          let nx=tx+d.x, ny=ty+d.y;
          if(ny>=0 && ny<H){
            nx = wrapX(nx);
            if(!isWall(nx,ny) && (passGate || !isGate(nx,ny))) return rev;
          }
        }
        return fromDir || 'left';
      }
...
try{ if(window.__pendingFruit && sceneRef && sceneRef.add){ window.__pendingFruit.forEach(a=>{ try{ makeFruit.apply(null,a);}catch(_){}}); window.__pendingFruit=[];} }catch(e){}
      ...
      const ovp = document.querySelector('#overlay .muted'); if(ovp){ ovp.textContent = 'Arrow Keys/WASD • M: mute • P: pause • R: restart. Eat all dots. Power orbs let you chomp ghosts (for a bit!).'; }
    ...
    function makePac(scene, tx, ty){
      ...
      function draw(){
        ...
        // Body as a large sector (circle minus mouth) �" works in WebGL & Canvas
        ...
        // Draw the large arc from (rot - mouth) to (rot + mouth) going anticlockwise to get 2π - 2*mouth span
        g.slice(px, py, Math.max(0, r), rot - mouth, rot + mouth, true);
        g.fillPath();
        
        // Eye �" lock to TOP when moving sideways; otherwise keep 90° CCW of heading
...
trail.push({x:px, y:py}); if(trail.length>18) trail.shift(); trailG.clear(); for(let i=1;i<trail.length;i++){ const a = i/trail.length; trailG.lineStyle(6, 0xffea00, 0.06*a); trailG.beginPath(); trailG.moveTo(trail[i-1].x, trail[i-1].y); trailG.lineTo(trail[i].x, trail[i].y); trailG.strokePath(); }
    ...
    function makeGhost(scene, tx, ty, color, corner, gi){
      ...
      if(!released){
        ...
        const now = Date.now();
        const sinceStart = (typeof startTime!=='undefined') ? (now - startTime) : 0;
        const readyToRelease = now >= holdInPenUntil;
        const allowByDots = readyToRelease && (pelletsEaten >= (thresholds[gi]||0));
        const allowByTime = readyToRelease && (sinceStart >= (timersMs[gi]||0));
